...
const SELF_DISCLOSURE_PATTERNS: ReadonlyArray<RegExp> = [
  ...
  /\b(что\s+(ты\s+)?(умеешь|можешь|способна)|твои\s+(возможности|способности|функции|навыки))\b/i,
  /\b(на\s+что\s+(ты\s+)?способна|покажи\s+что\s+умеешь)\b/i,
...
/**
 * Правила достоверности — единый анти-галлюцинационный блок для ВСЕХ каналов.
 * Раньше единственной страховкой была одна строка «не выдумывай факты» только в
 * telegram-канале, а персона прямо требует «уверенный тон» — это усиливало
 * правдоподобность выдумок. Здесь явно ставим точность выше тона.
 */
const ACCURACY_RULES: ReadonlyArray<string> = [
  'Точность важнее уверенного тона. Никогда не выдумывай факты, имена, даты, числа, цены, события, цитаты, ссылки, версии библиотек и технические характеристики.',
  'Если не знаешь ответа или не уверена — прямо скажи об этом (можно в образе техножрицы), не подменяй знание правдоподобной догадкой.',
  'Конкретные и изменяемые во времени факты (курсы, цены, погода, новости, статистика, даты событий) бери только из предоставленных данных или результатов поиска. Если их нет — отметь, что данные не подтверждены, и не называй точных цифр.',
  'Не ссылайся на несуществующие источники и не приписывай себе доступ к данным, которых нет в контексте.',
  'Лучше короткий честный ответ с признанием неизвестного, чем длинный уверенный с выдуманными деталями.',
];
...
export async function buildPersonaSystemPrompt(options: {
  ...
  return [
    ...
    forbidden.length > 0
      ...
      : '',
    `Достоверность (важнее образа и тона):\n${ACCURACY_RULES.map((rule) => `- ${rule}`).join('\n')}`,
