...
const IMAGE_EDIT_PATTERNS = [
  /(?:^|\s)(измени|отредактируй|исправь|переделай|перерисуй|поменяй|смени|замени|подправь)(?:\s|$)/i,
  /(?:^|\s)(убери|удали|уберите|удалите|вырежи|вырежьте)(?:\s|$)/i,
  /(?:^|\s)(добавь|добавьте|вставь|вставьте|дорисуй|нарисуй|напиши)(?:\s|$)/i,
  /(?:^|\s)(сделай|сделайте)(?:\s|$)/i,
  /(?:^|\s)(обрежь|поверни|отзеркаль|переверни|увеличь|уменьши|растяни|сожми|разверни)(?:\s|$)/i,
  /(?:^|\s)(перекрась|покрась|раскрась|закрась)(?:\s|$)/i,
  /(?:^|\s)(фон|задний план|бэкграунд)(?:\s|$)/i,
  /(?:^|\s)(стилизуй|стилизовать|в стиле|как у|эффект)(?:\s|$)/i,
  /(?:^|\s)(улучши|улучшить|апскейл|upscale|качество)(?:\s|$)/i,
  /(?:^|\s)(ярче|темнее|контрастнее|чётче|четче|резче|светлее|теплее|холоднее)(?:\s|$)/i,
...
export function extractEditPrompt(text: string): string {
  let prompt = text.trim();
  // Убираем вводные слова и обращения
  ...
  // Убираем "пожалуйста" из любого места
  prompt = prompt.replace(/[,\s]*пожалуйста[,\s]*/gi, ' ');
  // Убираем указания на картинку
  ...
  // Убираем вежливые слова в конце
  prompt = prompt.replace(/[,\s]*(плиз|please)[.!?]*$/i, '');
