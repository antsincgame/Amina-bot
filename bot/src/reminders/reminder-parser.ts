 ...
 *  Используем (?<=\s|^) / (?=\s|$) вместо \b — JS \b не работает с кириллицей. */
const REMINDER_LIST_PATTERNS = [
  /(?<=\s|^)(список|покажи|предоставь|выведи|открой|просмотр)(?=\s|$).{0,15}(напомин|remind)/i,
  /(?<=\s|^)(мои|все|активн|текущ)(?=\s|$).{0,10}(напомин|remind)/i,
  /(?<=\s|^)(напомин|remind).{0,10}(список|все|мои|покажи|сколько|какие|есть)(?=\s|$)/i,
  /^(напоминания|reminders)$/i,
  /(?<=\s|^)what.{0,10}remind/i,
