...
async function buildDigest(
  ...
  if (allAiHeadlines.length > 0) {
    const aiLines = allAiHeadlines.slice(0, 20).map(h =>
      `- ${h.title} (ссылка: ${h.url}) [источник: ${h.source}]${h.language && h.language !== 'ru' ? ` [язык: ${h.language}]` : ''}`
    );
    rawData.push(
      `[ТЕХНОЛОГИИ И AI — МЕЖДУНАРОДНЫЕ ИСТОЧНИКИ]\n` +
...
5. **Технологии и AI** — ЭКСПЕРТНЫЙ РАЗБОР как журналист-аналитик:
   - Выбери 5-8 САМЫХ ЗНАЧИМЫХ новостей из спарсенных AI/Tech заголовков
   - Для каждой: переведи на русский (если на английском), дай экспертный комментарий 2-3 предложения
