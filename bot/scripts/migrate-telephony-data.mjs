...
function mapScenario(r) {
  return {
    ...
    policy: trunc(JSON.stringify(r.policy ?? {}), 3000),
    goal: trunc(r.goal ?? '', 1500),
    system_prompt: trunc(r.system_prompt ?? '', 3000),
    opening_line: trunc(r.opening_line ?? '', 1500),
    question_hint: trunc(r.question_hint ?? '', 1500),
    success_criteria: trunc(r.success_criteria ?? '', 1500),
    result_prompt: trunc(r.result_prompt ?? '', 3000),
...
function mapSession(r) {
  return {
    ...
    scenario_goal: trunc(r.scenario_goal ?? '', 800),
    call_mode: r.call_mode,
    runtime_mode: r.runtime_mode ?? 'scripted',
    policy_version: r.policy_version ?? 1,
    provider: r.provider ?? 'unknown',
    target_phone: r.target_phone,
    task: trunc(r.task ?? '', 800),
    summary: trunc(r.summary ?? '', 2000),
    success_criteria: trunc(r.success_criteria ?? '', 800),
    result_prompt: trunc(r.result_prompt ?? '', 1000),
    request_id: r.request_id || null,
    request_mode: r.request_mode ?? '',
    call_id: r.call_id || null,
    record_link: trunc(r.record_link || null, 255),
    transcript: trunc(r.transcript || null, 2500),
    result_summary: trunc(r.result_summary || null, 1200),
