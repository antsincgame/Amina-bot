...
function mapSession(r) {
  return {
    ...
    scenario_goal: trunc(r.scenario_goal ?? '', 1200),
    call_mode: r.call_mode,
    runtime_mode: r.runtime_mode ?? 'scripted',
    policy_version: r.policy_version ?? 1,
    provider: r.provider ?? 'unknown',
    target_phone: r.target_phone,
    task: trunc(r.task ?? '', 1200),
    summary: trunc(r.summary ?? '', 3000),
    success_criteria: trunc(r.success_criteria ?? '', 1200),
    result_prompt: trunc(r.result_prompt ?? '', 2000),
    request_id: r.request_id || null,
    request_mode: r.request_mode ?? '',
    call_id: r.call_id || null,
    record_link: trunc(r.record_link || null, 500),
    transcript: trunc(r.transcript || null, 4000),
    result_summary: trunc(r.result_summary || null, 2000),
